...
export const SelectDropdown = ({ name, options, placeholder = 'Select', value, onChange, className = '', errors,disabled }) => {
    ...
    return (
        <div ref={dropdownRef} className={`relative ${className}`}>
            <button type="button" disabled={disabled} onClick={() => setIsOpen(!isOpen)} className={`flex justify-between items-center w-full border ${errors?.[name] ? 'border-red-500' : 'border-[#E1E4EA]'} rounded-lg px-3 py-2 bg-white text-left focus:outline-none focus:ring-1 focus:ring-[#675FFF]`}>
