 ...
 *
 * - Base URL comes from `VITE_AGENT_BASE_URL` environment variable.
...
agentInstance.interceptors.request.use(
  (config) => {
...
agentInstance.interceptors.response.use(
  (response) => response,
  async (error) => {
    ...
    if (
      ...
      try {
        ...
        agentInstance.defaults.headers['Authorization'] = `Bearer ${newAccessToken}`;
        originalRequest.headers['Authorization'] = `Bearer ${newAccessToken}`;

        // Retry the original request
        return agentInstance(originalRequest);
      } catch (refreshError) {
